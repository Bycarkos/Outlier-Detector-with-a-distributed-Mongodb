...
class DMOD(od):
  ...
  def __updateGDMOD(self, i, X, G, M, S, H, Y, mu=1, beta=1):
    ...
    for m in range(numSamples):
      cost = [0] * K
      for j in (k for j in (range(i), range(i + 1, numViews)) for k in j):
        if i < j:
          pos = int(j * (j - 1) / 2 + i)
        else:
          pos = int(i * (i - 1) / 2 + j)
  ...
  def detector(self, FeaturesList, y_train=None , **params): #k=2, beta=0.6, gamma=0.01):
    '''
    ...
    X = []
    G = []
    M = [np.eye(k)] * int((numViews * (numViews - 1) / 2))
    H = []
    S = []
    Y = []
    for taula in FeaturesList:
      ...
      kmeans = KMeans(n_clusters=k).fit(featureMatrix)
    ...
    while E > self.eps and it < self.maxIter:
      ...
      # Update G
      for i in range(numViews):
        G[i] = self.__updateGDMOD(i, X, G, M, S, H, Y, mu=mu, beta=beta)

      # Update M
      for i in range(numViews):
        for j in range(i + 1, numViews):
          a = np.matmul(G[i], np.transpose(G[j])) + 10 ** -16
          M[int(j * (j - 1) / 2 + i)] = np.matmul(np.diag(1.0 / np.sum(a, axis=1)), a)
    ...
    # Outlier measurement
    out = 1-self.score(G, S, gamma=gamma)
